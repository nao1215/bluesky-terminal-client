...
The suite runs on Linux, macOS, and Windows, every scenario on all three (on Windows atago draws `graphics: kitty` through the OpenConsole it ships). Skip a platform only where bsky itself behaves differently there, and say why in a comment.
