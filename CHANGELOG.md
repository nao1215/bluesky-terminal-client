...
- A Notifications tab (`4`) shows likes, reposts, follows, mentions, replies, and quotes with the posts they are about, counts the unread ones on the tab, and marks them seen up to the newest one shown, in the server's time rather than the local clock. Replies, mentions, and quotes can be answered and liked from it.
...
- Nine color themes (`default`, `light`, `dracula`, `nord`, `gruvbox`, `solarized`, `catppuccin`, `tokyo-night`, `monochrome`) in a picker opened with `T`, previewed live and saved in `settings.json`. A `settings.json` that cannot be read is reported and never overwritten. 24-bit themes are mapped to 256 colors on terminals without 24-bit color, and `NO_COLOR` turns color off.
...
- One video (MP4, MOV, WebM, MPEG; up to 100 MB and 3 minutes) or animated GIF on a post, with alt text and its shape read from the file's header. It is uploaded through Bluesky's video service with a token the PDS issues for it, and bs waits for the service to process it; the service turns a GIF into a video, so bs needs no codec. The browser and the composer describe a video by its length, shape, and size instead of playing it, and a post has up to four pictures or one video, not both. Videos on the timeline are shown by their thumbnail.
- Faster scrolling: pictures are scaled and encoded for the terminal on background threads instead of while drawing, the pictures of the next posts are prepared before they scroll into view, and keys pressed faster than a frame draws are applied together. Downloaded pictures are kept on disk (`BS_CACHE_DIR`, 256 MB, least recently used removed first), so the next start draws them without downloading.
