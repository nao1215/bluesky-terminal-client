//! The Columns tab: lists the client already has, side by side. Each column
//! is one source (the following timeline, a pinned feed, the notifications,
//! a search, an account's posts) with a list of its own, loaded and paged as
//! the same list is on its own tab. Which columns there are is kept in
//! `settings.json`, per account.
